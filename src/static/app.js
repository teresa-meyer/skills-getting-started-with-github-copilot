document.addEventListener("DOMContentLoaded", () => {
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    if (deleteButton) {
      const item = deleteButton.closest(".participant-item");
      if (!item) {
        return;
      }

      const confirmed = window.confirm(
        `Remove ${item.dataset.email} from ${item.dataset.activity}?`
      );
      ...
      await unregisterParticipant(item.dataset.activity, item.dataset.email);
      return;
    }
  });
